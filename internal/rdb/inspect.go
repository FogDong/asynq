...
// Returns -2 if task is failed to add to cancelled set.
...
redis.call("HSET", KEYS[1], "msg", ARGV[3], "state", "cancelled")
return 1
`)
...
func (r *RDB) CancelTask(qname, id string) error {
	...
	fmt.Println("cancelTaskCmd returned", n)
	switch n {
	case 1:
		return nil
	case 0:
	...
	case -2:
		return errors.E(op, errors.Internal, fmt.Sprintf("failed to add task to cancelled set: %s", msg.ID))
	default:
