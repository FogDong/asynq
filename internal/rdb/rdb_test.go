...
func TestCancelTask(t *testing.T) {
	...
	cancelledKey := base.CancelledKey(queueName)
	cancelledIDs := r.client.ZRange(context.Background(), cancelledKey, 0, -1).Val()
	if n := len(cancelledIDs); n != 2 {
		t.Errorf("Redis LIST %q contains %d IDs, want 2", cancelledKey, n)
	}
...
func newCancelledTask(qname, typename string, payload []byte, cancelledAt time.Time) *base.TaskMessage {
	msg := h.NewTaskMessageWithQueue(typename, payload, qname)
	msg.CancelledAt = cancelledAt.Unix()
	return msg
}

func TestDeleteExpiredCompletedAndCancelledTasks(t *testing.T) {
	r := setup(t)
	defer r.Close()
	now := time.Now()
	secondAgo := now.Add(-time.Second)
	...
	t6 := newCancelledTask("default", "cancelled_task", nil, hourAgo)

	tests := []struct {
		desc          string
		completed     map[string][]base.Z
		cancelled     map[string][]base.Z
		qname         string
		wantCompleted map[string][]base.Z
		wantCancelled map[string][]base.Z
	}{
		{
	...
	for _, tc := range tests {
		...
		for qname, want := range tc.wantCancelled {
			got := h.GetCancelledEntries(t, r.client, qname)
			if diff := cmp.Diff(want, got, h.SortZSetEntryOpt); diff != "" {
				t.Errorf("%s: diff found in %q cancelled set: want=%v, got=%v\n%s", tc.desc, qname, want, got, diff)
			}
		}
	}
}
