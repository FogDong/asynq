...
func (tb *TestBroker) DeleteExpiredCompletedAndCancelledTasks(qname string, batchSize int) error {
	tb.mu.Lock()
	defer tb.mu.Unlock()
	if tb.sleeping {
		return errRedisDown
	}
	return tb.real.DeleteExpiredCompletedAndCancelledTasks(qname, batchSize)
}
